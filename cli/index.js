...
program
  .command('mcp')
  .description('Start Model Context Protocol (MCP) server on stdin/stdout')
  .action(() => {
        ...
        case 'tools/list':
          sendResultResponse(id, {
            tools: [
              {
                name: 'get_status',
                description: 'Get ContextLens CLI status, linked project ID, and active episode ID',
                inputSchema: {
                  type: 'object',
                  properties: {}
                }
              ...
              {
                name: 'start_episode',
                description: 'Start a new tracking episode in the current branch',
                inputSchema: {
                  type: 'object',
                  properties: {
              ...
              {
                name: 'close_episode',
                description: 'Close the currently active episode in ContextLens',
                inputSchema: {
                  type: 'object',
                  properties: {}
                }
        ...
        switch (name) {
          case 'get_status': {
            ...
            const responseText = [
              ...
              `Active Episode ID: ${config.episodeId || '❌ None'}`,
              `Project Root: ${projectRoot || 'N/A'}`
            ].join('\n');

            sendToolResult(id, responseText);
            break;
          }
          ...
          case 'start_episode': {
            ...
            if (!token) {
              sendToolError(id, 'Not authenticated. Run "cl auth <token>" first.');
              return;
            }
            if (!projectId) {
              sendToolError(id, 'No project configured. Run "cl init <project-id>" first.');
              return;
            }

            const label = args?.label || `MCP Session ${new Date().toISOString().slice(0, 10)}`;
            let branchName = 'main';
            try {
              branchName = execSync('git rev-parse --abbrev-ref HEAD', { stdio: ['pipe', 'pipe', 'pipe'] }).toString().trim() || 'main';
            } catch {}
            ...
            sendToolResult(id, `Successfully created new episode: "${label}" (ID: ${episodeId})`);
            break;
          }
          ...
          case 'close_episode': {
            ...
            if (!token) {
              sendToolError(id, 'Not authenticated.');
              return;
            }
            if (!projectId || !episodeId) {
              sendToolError(id, 'No active project or episode.');
              return;
            }

            await apiRequest('/episodes/close', 'POST', {
              projectId,
              episodeId
            }, token, apiBaseUrl);

            saveConfig({ episodeId: null }, false);
            sendToolResult(id, `Successfully closed episode ${episodeId}`);
            break;
          }
          ...
          case 'log_ai_call': {
            ...
            if (!projectId) {
              sendToolError(id, 'No project configured.');
              return;
            }

            if (!episodeId) {
              const label = `MCP Auto Session ${new Date().toISOString().slice(0, 10)}`;
              let branchName = 'main';
              try {
                branchName = execSync('git rev-parse --abbrev-ref HEAD', { stdio: ['pipe', 'pipe', 'pipe'] }).toString().trim() || 'main';
              } catch {}
            ...
            sendToolResult(id, `AI call logged successfully! (Call ID: ${result.callId}, Episode ID: ${episodeId})`);
            break;
          }
          ...
          case 'explain_diff': {
            ...
            const result = await apiRequest('/episodes/explain', 'POST', {
              projectId,
              episodeId
            }, token, apiBaseUrl);
            ...
            const responseText = [
              ...
              `**Summary:**`,
              result.summary || 'No changes to explain or no analysis generated.',
              `\n**Risks Identified:**`,
              (result.risks && result.risks.length > 0) ? result.risks.map(r => `- ${r}`).join('\n') : '- None',
              `\n**Suggested Checks:**`,
              (result.checks && result.checks.length > 0) ? result.checks.map(c => `- ${c}`).join('\n') : '- None'
            ].join('\n');
          ...
          case 'search_context': {
            ...
            const res = await apiRequest('/search', 'POST', {
              projectId,
              q: args.query
            }, token, apiBaseUrl);
          ...
          case 'get_episode_details': {
            ...
            const res = await apiRequest('/episodes/get', 'POST', {
              projectId,
          ...
          case 'get_recent_episodes': {
            ...
            const res = await apiRequest('/episodes/list', 'POST', {
              projectId,
              limit: args.limit
            }, token, apiBaseUrl);
          ...
          case 'explain_past_changes': {
            ...
            const res = await apiRequest('/episodes/explain', 'POST', {
              projectId,
              episodeId: args.episodeId
            }, token, apiBaseUrl);
